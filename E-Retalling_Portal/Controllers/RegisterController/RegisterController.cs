...
namespace E_Retalling_Portal.Controllers.AccountController
{
    public class RegisterController : Controller
    {
        ...
        public IActionResult UserInformationRegisterCheck([MaxLength(100)] String displayName,
                                                          ...
                                                          [Range(1, int.MaxValue)]int province,
                                                          [Range(1, int.MaxValue)]int district,
                                                          ...
                                                          [DataType(DataType.Password), MaxLength(100)] String passwordConfirm)
        {
            ...
            if (ModelState.IsValid == true)
            {
                using (var context = new Context())
                {
                    User newUser = new User { email = email, phoneNumber = phoneNumber, displayName = displayName, birthday = birthday, gender = gender, firstName = firstName, lastName = lastName, province = province, district = district, ward = ward, address = address };
                    context.Add(newUser);
                    context.SaveChanges();
