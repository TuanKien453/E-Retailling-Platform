...
{
    public class SellerForgotPasswordController : Controller
    {
        ...
        public IActionResult SendOTP(string emailTo)
        {
            using (var context = new Context())
            {
                try
                {
                    var account = context.Accounts.GetAccountByRoleIdAndUserId(2, context.Users.GetUserByEmail(emailTo).FirstOrDefault()).FirstOrDefault();
                }
                catch (Exception ex)
                {
        ...
        public IActionResult ChangePassword(string password, string email)
        {
            using (var context = new Context())
            {
                var account = context.Accounts.GetAccountByRoleIdAndUserId(2, context.Users.GetUserByEmail(email).FirstOrDefault()).FirstOrDefault();
