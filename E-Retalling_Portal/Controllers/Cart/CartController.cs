        ...
        public async Task<IActionResult> Index(int? page)
        {
            using (var context = new Context())
            {
                ...
                var cartDetails = cartItems.Select(ci => new CartItemModel
                {
                    ...
                    discountedPrice = ci.Key.EndsWith("P")
    ? products.FirstOrDefault(p => p.deleteAt == null && p.id == int.Parse(ci.Key.TrimEnd('P')))
        != null ? (float)context.Products.GetProductDiscountPrice(
            products.FirstOrDefault(p => p.deleteAt == null && p.id == int.Parse(ci.Key.TrimEnd('P'))))
        : 0
    : ci.Key.EndsWith("PI")
        ? productItems.FirstOrDefault(pi => pi.deleteAt == null && pi.id == int.Parse(ci.Key.TrimEnd("PI".ToCharArray())))
            != null ? (float)context.ProductItems.GetProductItemDiscountPrice(
                productItems.FirstOrDefault(pi => pi.deleteAt == null && pi.id == int.Parse(ci.Key.TrimEnd("PI".ToCharArray()))))
            : 0
        : 0
